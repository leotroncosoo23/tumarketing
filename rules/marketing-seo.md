...
- **Server-Side Rendering (SSR) y Static Site Generation (SSG):** Todas las páginas públicas de blogs, guías gratuitas y landing pages de servicios DEBEN ser renderizadas en el servidor. Utilizar la API de Next.js (`generateMetadata`) para asegurar que los motores de búsqueda puedan leer el contenido de inmediato.
...
- **Jerarquía HTML:** Respetar estrictamente la semántica web. Debe haber un único `<h1>` por página, seguido de un uso lógico de `<h2>` y `<h3>` para estructurar los artículos y descripciones de servicios.
- **Sitemap y Robots:** Estructurar el código para que sea fácil generar el `sitemap.xml` dinámico y gestionar el `robots.txt`, asegurando que las páginas privadas (como `/admin` o el portal del usuario) estén excluidas de la indexación.